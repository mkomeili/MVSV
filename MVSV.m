...
% lambda: it is hyper-parameter that controls the cross-view matching error. See the paper.
%
%Output:
% w1: feature weight vector for view x. 
% w2: feature weight vector for view y.

function [w1, w2] = MvFS(X,Y,id, sigma,lambda)
...
while  (Difference>0.01) && (t<=4) 
    ...
    [w, fval]  = GD_solver( w,P,Sm,lambda,itr_max );
